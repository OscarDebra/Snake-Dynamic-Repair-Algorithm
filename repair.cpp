...
deque <Vector2> FindHamiltonianPath() {
    ...
    while (visited.size() < totalSquares || currentCoord != endCoord) {
        ...
        deque <Vector2>& moves = availableMoves[currentCoord];
        ...
        if (!moves.empty()) {
            ...
            availableMoves.erase(currentCoord); // Clear this position's moves, for if we come back to this tile via a different path.
            
            visited.pop_back();
            currentCoord = visited.back();
        }
    }
